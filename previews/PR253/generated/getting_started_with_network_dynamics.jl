...
# For example `idxs = VIndex(1, :v)` accesses the state `:v` of vertex 1. See Symbolic Indexing for more details.

plot(sol; idxs=vidxs(nd, :, :), fmt=:png)

# In order to collect multiple indices we can use the helper function `vidxs` and `eidxs`, which help to
