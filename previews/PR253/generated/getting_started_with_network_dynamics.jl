...
#
# This example can be dowloaded as a normal Julia script [here](getting_started_with_network_dynamics.jl).
#
...
# its difference with its neighbors and we obtain the following ordinary differential equation
#
# ```math
...
# reduces allocations and thereby speeds up computations. After the function call the edge's output value `e` equals
# the difference between its source and its destination vertex (i.e. the discrete gradient along that edge).
#
...
#
# The output function `g` is just taking part of the internal states. For that we can use the `StateMask` helper
...
# state `:v` of vertex 1. See Symbolic Indexing for more details.
#
# In oder to collect multiple indices we can use the helper function `vidxs` and `eidxs`, which help to
