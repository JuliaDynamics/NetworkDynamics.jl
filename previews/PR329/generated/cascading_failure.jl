...
# For that we use a `PresetTimeComponentCallback`, which triggers an
...
# Now we can simulate the network. The `ODEProblem`)
...
# > arguments like `add_comp_cb`, `add_nw_cb`, and `override_cb`. See the `ODEProblem`)
# > documentation for details.
#
# Lastly we plot the power flow on all lines using the `eidxs` function to generate the
...
# manually what `get_callbacks(::Network)` is doing for us.
#
