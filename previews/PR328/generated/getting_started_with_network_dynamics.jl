...
# model will be comprised of nodes that have no internal dynamics. This means that the evolution of a node will depend
# only on its interaction with its neighbors. In NetworkDynamics.jl, interactions between a node and its neighbors are
# described using edge equations.
#
# In order to bring this equation into the form required by NetworkDynamics.jl we need to split the dynamics into edge
...
# flows connected to the vertex. This directly corresponds to the component model definition outlined in Mathematical Model:
# ```math
# \begin{aligned}
...
# `AntiSymmetric` output wrapper to construct the `EdgeModel`:
...
#     can use the `StateMask` helper function `g = StateMask(1:1)` to access them.
...
# components and their symbols directly (see Symbolic Indexing for more details). In order to collect multiple
# indices we can use the helper function `vidxs` and `eidxs`, which help us collect all symbolic indices
...
#
# To illustrate a very simple multi-dimensional case, we simulate two independent diffusions on an identical graph.
# The first diffusion uses the symbol `x` and is initiated with initial conditions drawn from the standard normal
