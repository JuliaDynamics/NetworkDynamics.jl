...
NDPATH = begin
    ...
    i = findlast(contains("NDPrototype"), path)
...
therefore we copy the repo to tmp/NDPrototyp
We want to have the same benchmarks for target and base therefore we copy
NDPrototyp/benchmark to tmp/benchmark so it won't change if the repo
state changes.
=#
tmp = tempname(); mkpath(tmp)
ndpath_tmp = joinpath(tmp, "NDPrototyp")
