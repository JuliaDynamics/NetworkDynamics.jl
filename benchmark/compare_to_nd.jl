...
NDPATH = begin
    ...
    i = findlast(isequal("NDPrototype"), path)
...
therefore we copy the repo to tmp/NDPrototype
We want to have the same benchmarks for target and base therefore we copy
NDPrototype/benchmark to tmp/benchmark so it won't change if the repo
state changes.
=#
tmp = tempname(); mkpath(tmp)
ndpath_tmp = joinpath(tmp, "NDPrototype")
