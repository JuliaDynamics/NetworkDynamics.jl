...
# But there is also a built-in method `set_interface_defaults!` which we can use
# automatically:
...
# are left "free". Using `initialize_component!`, we can try to find values for the
# "free" states and parameters such that the interface constraints are fulfilled.

initialize_component!(nw_dyn[VIndex(1)])

# We may also use `dump_initial_state` to get a more detailed view of the state:
