...
function _get_metadata(sys, name, alldefaults, allguesses)
    ...
    default_from = Symbolics.getmetadata(unwrap(sym), NetworkDynamics.ParameterDefaultFrom, nothing)
    if !isnothing(default_from)
        # `[default_from = :busbar₊Vbase]` arrives as a `Symbol`, `[default_from =
        # (:src, :busbar₊Vbase)]` as a `Tuple` of `Symbol`s (QuoteNode-wrapped in some cases)
        md[:default_from] = _normalize_default_from(default_from)
    end
    md
end
