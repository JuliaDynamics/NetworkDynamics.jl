...
# pressure.
# Additionally, we add some "internal" state `q̃_inj` which we want to plot later (see also Observables).
...
#
# We can use the `find_fixpoint` from `NetworkDynamics.jl` to initialize the system.
# Internally, this uses a nummerical solve for the rootfind problem `0 = rhs`.
# The result is automaticially wrapped as a `NWState` object.
