...
function (d::dynamic_line_network_dynamics)(dx, x, p, t)
    for i in 1:d.len_l
        d.lines![i](d.l_e[i],view(x,d.len_n+i), x[d.s_e[i]], x[d.t_e[i]], p, t)
...
function (d::multi_static)(dx, x, p, t)
    for i in 1:d.len_l
        for j in 1:d.len_n_var
            d.lines![i,j](d.l_e[i,j], x[(d.s_e[i]-1)*d.len_n_var + j], x[(d.t_e[i]-1)*d.len_n_var + j], p, t)
        end
    end
    for i in 1:d.len_n
        for j in 1:d.len_n_var
            d.nodes![i,j](view(dx,i*d.len_n_var + j-1), view(x,(i*d.len_n_var):(i*d.len_n_var+j-1)), d.l_s[i,j], d.l_t[i,j], p, t)
        end
    end
    nothing
end
...
function multi_static(nodes!, lines!, s_e, t_e)
    len_l = length(lines!)
    len_n = length(nodes!)
    len_n_var=length(nodes![1])

    # Will get longer once we have more variables per line
    l_e_int = rand(len_l,len_n_var)
    ...
    multi_static(nodes!, lines!, s_e, t_e, l_e, l_e_int, l_s, l_t, len_l, len_n,len_n_var)
end
