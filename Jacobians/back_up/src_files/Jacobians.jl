...
struct JacGraphData
    ...
    #e_jac_product::Array{Float64, 2} with this it works
...
function JacGraphData(v_Jac_array::Tvj, e_Jac_array::Tej, e_Jac_product_array::Tep, gs::GraphStruct) where {Tvj, Tej, Tep}
    ...
    #e_jac_product =  zeros(gs.num_e, gs.e_dims[1]) # with this it works
...
function jac_vec_prod!(dx, Jac::NDJacVecOperator, z)
    ...
    for i in 1:gs.num_e
        ...
        #jgd.e_jac_product[i, :] .= get_src_edge_jacobian(jgd, i) * view(z, gs.s_e_idx[i]) + get_dst_edge_jacobian(jgd, i) * view(z, gs.d_e_idx[i]) # with this it works
        jgd.e_jac_product[i] .= get_src_edge_jacobian(jgd, i) * view(z, gs.s_e_idx[i]) + get_dst_edge_jacobian(jgd, i) * view(z, gs.d_e_idx[i])
    end
    ...
    for i in 1:gs.num_v
        ...
        #dx .+= sum(jgd.e_jac_product[i, :]) # with this it works
        dx .+= sum(jgd.e_jac_product[i])
    end
end
