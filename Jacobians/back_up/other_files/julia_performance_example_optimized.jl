#UPDATE: Thanks to many useful comments I was able to improve performance. The primary speed-up came from resolving type uncertainties by splitting things up into many smaller functions. There are still some lingering questions about avoiding memory allocation (the anonymous function "objective = h -> - bellman(h, stock, valu_interp)" appears to be the real killer). Below is the updated code.
